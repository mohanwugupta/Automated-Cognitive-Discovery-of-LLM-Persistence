...
def generate_design_main(argv=None):
    parser = argparse.ArgumentParser(description="Compile the declarative persistence design")
...
def run_experiments_main(argv=None):
    parser = argparse.ArgumentParser(description="Collect or finalize behavior-only experiments")
    parser.add_argument("--config", default=DEFAULT_CONFIG)
    parser.add_argument("--output")
    ...
    if args.phase == "collect":
        ...
        print(json.dumps({"observations": len(frame), "path": str(path), "gates": gates}, indent=2))
...
def validate_predictions_main(argv=None):
    parser = argparse.ArgumentParser(description="Collect independent conditions and score a frozen model")
    parser.add_argument("--config", default=DEFAULT_CONFIG)
    parser.add_argument("--output")
    parser.add_argument("--conditions", type=int)
    parser.add_argument("--model")
...
def discover_residuals_main(argv=None):
    parser = argparse.ArgumentParser(description="Discover held-out residual interactions")
    parser.add_argument("--config", default=DEFAULT_CONFIG)
    parser.add_argument("--output")
