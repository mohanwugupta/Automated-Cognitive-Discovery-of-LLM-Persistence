...
tests_submission=$(sbatch --parsable --job-name=cog_ood_tests --time=00:45:00 \
  --export=ALL,PHASE=tests "$CPU_SCRIPT")
tests_job="${tests_submission%%;*}"
prepare_submission=$(sbatch --parsable --job-name=cog_ood_prepare --time=02:00:00 \
  --dependency="afterok:${tests_job}" --export=ALL,PHASE=prepare "$CPU_SCRIPT")
prepare_job="${prepare_submission%%;*}"
dispatch_submission=$(sbatch --parsable --job-name=cog_ood_dispatch --time=00:15:00 \
  --dependency="afterok:${prepare_job}" --export=ALL,PHASE=dispatch "$CPU_SCRIPT")
dispatch_job="${dispatch_submission%%;*}"

echo "OOD free-generation workflow submitted"
echo "  CPU tests:            ${tests_job}"
echo "  CPU protocol freeze:  ${prepare_job}"
echo "  CPU exact dispatcher: ${dispatch_job}"
echo "Only cached Qwen generation and neural-control jobs request GPUs."
