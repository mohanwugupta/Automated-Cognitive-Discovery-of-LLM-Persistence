...
Post-run finite-precision audit: compare each random intervention's achieved norm with the frozen intervention's achieved norm. Report all mismatches. A secondary sensitivity analysis retains only pairs for which every random control lies within 5% of the frozen norm, then recomputes both recovery endpoints and all random comparisons on that common subset. This filter is based on norm matching, not recovery scores; it is post hoc and does not replace the full-data results.
